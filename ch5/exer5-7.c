/* Exercise 5-7, have main pass an array to readlines rather than calling alloc
    and compare speed to sortLines */
#include <stdio.h>
#include <string.h>
#include <time.h>
...
int readlines(char *lineptr[], int maxlines)
{
    ...
    while ((len = getLine(line, MAXLEN)) > 0)
        if (nlines >= maxlines || len >=  (allocbuf + ALLOCSIZE - allocp))
            return -1;
        else {
